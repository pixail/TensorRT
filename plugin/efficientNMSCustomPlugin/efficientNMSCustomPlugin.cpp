...
namespace
{
char const* kEFFICIENT_NMS_CUSTOM_PLUGIN_VERSION{"1"};
char const* kEFFICIENT_NMS_CUSTOM_PLUGIN_NAME{"EfficientNMSCustom_TRT"};
} // namespace
...
EfficientNMSCustomPlugin::EfficientNMSCustomPlugin(void const* data, size_t length)
{
    deserialize(static_cast<int8_t const*>(data), length);
}

EfficientNMSCustomPlugin::deserialize(int8_t const* data, size_t length)
{
    auto const* d{data};
...
char const* EfficientNMSCustomPlugin::getPluginType() const noexcept
{
    return kEFFICIENT_NMS_CUSTOM_PLUGIN_NAME;
}

char const* EfficientNMSCustomPlugin::getPluginVersion() const noexcept
{
    return kEFFICIENT_NMS_CUSTOM_PLUGIN_VERSION;
}

int32_t EfficientNMSCustomPlugin::getNbOutputs() const noexcept
{
    // Standard Plugin Implementation
    return 5;
}

int32_t EfficientNMSCustomPlugin::initialize() noexcept
{
    if (!initialized)
    {
        int32_t device;
...
void EfficientNMSCustomPlugin::setPluginNamespace(char const* pluginNamespace) noexcept
{
    try
    {
...
char const* EfficientNMSCustomPlugin::getPluginNamespace() const noexcept
{
...
int32_t EfficientNMSCustomPlugin::enqueue(PluginTensorDesc const* inputDesc, PluginTensorDesc const* outputDesc,
...
char const* EfficientNMSCustomPluginCreator::getPluginName() const noexcept
{
    return kEFFICIENT_NMS_CUSTOM_PLUGIN_NAME;
}

char const* EfficientNMSCustomPluginCreator::getPluginVersion() const noexcept
{
    return kEFFICIENT_NMS_CUSTOM_PLUGIN_VERSION;
}

PluginFieldCollection const* EfficientNMSCustomPluginCreator::getFieldNames() noexcept
{
    return &mFC;
}

IPluginV2DynamicExt* EfficientNMSCustomPluginCreator::createPlugin(char const* name, PluginFieldCollection const* fc) noexcept
{
    try
    {
